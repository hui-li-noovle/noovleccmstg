include: "mat_dashboard.view.lkml"
...
view: +mat_dashboard {
  ...
  dimension: period_selected {
    ...
    sql:
        CASE
          WHEN ${mat_dashboard.usage_start_date} >=  ${first_date_in_period}
          THEN 'This {% parameter period %} to Date'
          WHEN ${mat_dashboard.usage_start_date} >= ${first_date_in_prior_period}
          AND ${mat_dashboard.usage_start_date} <= ${last_date_in_prior_period}
  ...
  dimension: days_from_period_start {
    ...
    sql: CASE WHEN ${period_selected} = 'This {% parameter period %} to Date'
          THEN DATE_DIFF(${mat_dashboard.usage_start_date}, ${first_date_in_period}, DAY)
          WHEN ${period_selected} = 'Prior {% parameter period %} to Date'
          THEN DATE_DIFF(${mat_dashboard.usage_start_date}, ${first_date_in_prior_period}, DAY)
          ELSE NULL END;;
  }
